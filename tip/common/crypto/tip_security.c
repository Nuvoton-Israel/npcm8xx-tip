							   ...
							   uint8_t hashAlg)
{
	...
	if (((uint32_t) imageAddr < TIP_DRAM_WIN0_BASE_ADDR) ||
		...
		nclStat_ret =
			pROM_SEC_SHA_Calc(shaCtx, (hashAlg == HASH_TYPE_SHA384 ? NCL_SHA_TYPE_2_384 : NCL_SHA_TYPE_2_512), imageAddr, imageLen, (uint8_t *) hash);
	} else {
		...
		pROM_NCL_SHA_Start (&shaCtx, (hashAlg == HASH_TYPE_SHA384 ? NCL_SHA_TYPE_2_384 : NCL_SHA_TYPE_2_512));
							  ...
							  uint8_t hashAlg)
{
	...
	*sigOK = SECURED_FALSE;
	...
	nclStat_ret = pROM_NCL_ECC_ECDSAVerifyHash (eccCtx, &nclStat, &rSig, &sSig, (hashAlg == HASH_TYPE_SHA384 ? NCL_SHA_TYPE_2_384: NCL_SHA_TYPE_2_512),
