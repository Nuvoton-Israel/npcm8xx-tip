...
void validate_internal_sig(const void *data,
    ...
    retStatus = lm_validate_signature(d->public_key,
                                      d->message, d->message_len, false,
                                      d->signature, d->signature_len);

    if (DEFS_SEC_COND_CHECK(retStatus == DEFS_STATUS_OK))
    {
        return;
    }
 ...
 */
DEFS_STATUS hss_validate_signature(
    ...
    volatile enum hss_error_code sec_error = hss_error_bad_signature;
    ...
    if (DEFS_SEC_COND_CHECK(sec_error == hss_error_none))
    {
    ...
    DEFS_FLOW_MONITOR_COMPARE_RET(4);

    return retStatus;

failed:           /* If we get an intermediate failure */
    hss_thread_done(col);
    return DEFS_STATUS_AUTHENTICATION_FAIL;
}
