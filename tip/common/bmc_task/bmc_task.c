...
static void tip_bmc_direct_finalize_command (int status, UINT32 notification)
{
	...
	if (notification == BMC_DIRECT_NOTIFCATION_FL) {
		...
		REG_WRITE (SCRPAD_10_41 (21), status);
	}
	else if (notification == BMC_DIRECT_NOTIFCATION_AES) {
		REG_WRITE (SCRPAD_10_41 (26), status);
	}
...
void NVIC_BMC_direct_handler (uint16_t num)
{
	...
	platform_printf_dbg (KGRN NEWLINE
		...
		int_num, notification, REG_READ (SCRPAD_10_41 (10)));
	...
	if (cmd_reg_ind != 0) {
		...
		platform_printf_dbg (KMAG "cmd %x cmd_ind %d" NEWLINE KNRM, cmd, cmd_reg_ind);
	...
	/* check if it's a BMC_DIRECT RNG command */
		...
		uint32_t cmd = REG_READ (SCRPAD_10_41 (21));
		/* prevent reuse of command. FFFFFFF means command is being executed. */
		REG_WRITE (SCRPAD_10_41 (21), 0xFFFFFFFF);
		if ((cmd > BMC_RESET_0) && (cmd < BMC_MAX_COMMANDS)) {
			xTaskNotifyFromISR (bmc_reset_task.bmc_task, cmd, eSetValueWithOverwrite,
				&reset_priority);
			return;
			...
			switch (notification) {
				...
				case BMC_DIRECT_COMMAND_FL_READ_PARMAS: {
					...
					if (fl != NULL) {
						...
						REG_WRITE (SCRPAD_10_41 (13), size);

						spi_flash_get_sector_size (fl, &size);
						REG_WRITE (SCRPAD_10_41 (14), size);

						spi_flash_get_block_size (fl, &size);
						REG_WRITE (SCRPAD_10_41 (15), size);
				...
				case BMC_DIRECT_COMMAND_DRBG: {
					uint32_t num_of_random_bytes = REG_READ (SCRPAD_10_41 (22));
					uint32_t addr = REG_READ (SCRPAD_10_41 (23));
				...
				case BMC_DIRECT_COMMAND_AES_DEC_GCM: {
					...
					uint32_t addr_src = REG_READ (SCRPAD_10_41 (24));
					uint32_t addr_dst = REG_READ (SCRPAD_10_41 (27));
					uint32_t size = REG_READ (SCRPAD_10_41 (25));

					struct info *iv_info = (struct info *)REG_READ (SCRPAD_10_41 (28));
					struct info *key_info = (struct info *)REG_READ (SCRPAD_10_41 (29));
					struct info *tag_info = (struct info *)REG_READ (SCRPAD_10_41 (30));
					...
					platform_printf_dbg ( "scrpad24 %#010lx %#010lx \n", REG_ADDR (SCRPAD_10_41 (24)),  REG_READ(SCRPAD_10_41 (24)));
					platform_printf_dbg ( "scrpad27 %#010lx %#010lx \n", REG_ADDR (SCRPAD_10_41 (27)),  REG_READ(SCRPAD_10_41 (27)));
					platform_printf_dbg ( "scrpad28 %#010lx %#010lx \n", REG_ADDR (SCRPAD_10_41 (28)),  REG_READ(SCRPAD_10_41 (28)));
					platform_printf_dbg ( "scrpad29 %#010lx %#010lx \n", REG_ADDR (SCRPAD_10_41 (29)),  REG_READ(SCRPAD_10_41 (29)));
