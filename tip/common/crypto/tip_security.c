...
#ifdef LMS_ENABLE
#define TIP_ROM_A3_ADDR_NCL_LMS_VerifyData             ((ROM_NCL_LMS_VerifyData)(0x00010228  + THUMB_BIT))
#define TIP_ROM_A3_ADDR_NCL_LMS_InitContext            ((ROM_NCL_LMS_InitContext)(0x0001021c  + THUMB_BIT))
#define TIP_ROM_A3_ADDR_NCL_LMS_Init                   ((ROM_NCL_LMS_Init)(0x00010234  + THUMB_BIT))
#define TIP_ROM_A3_ADDR_NCL_LMS_FinalizeContext        ((ROM_NCL_LMS_FinalizeContext)(0x00010220  + THUMB_BIT))
#define TIP_ROM_A3_ADDR_NCL_LMS_Clear                  ((ROM_NCL_LMS_Clear)(0x0001022c  + THUMB_BIT))
#endif
/**
...
ROM_NCL_DRBG_Config                pROM_NCL_DRBG_Config;
...
#ifdef LMS_ENABLE
ROM_NCL_LMS_VerifyData             pROM_NCL_LMS_VerifyData;
ROM_NCL_LMS_InitContext            pROM_NCL_LMS_InitContext;
ROM_NCL_LMS_Init                   pROM_NCL_LMS_Init;
ROM_NCL_LMS_FinalizeContext        pROM_NCL_LMS_FinalizeContext;
ROM_NCL_LMS_Clear                  pROM_NCL_LMS_Clear;
#endif
...
/**
 *  @function     tip_rng_optimization
 *  @brief        Initialize RNG relevan registers and configuration to optimze generation run time
 *  @return 0 if the ncl debug config has configured successfully or an error code.
 */
int tip_rng_optimization(void)
{
	pROM_NCL_DRBG_Config              = TIP_ROM_A1_ADDR_NCL_DRBG_Config;
	/*	Init RNG module (for devices with empty OTP NCL)*/
	REG_WRITE (RNGCS(0),     0x28);
	REG_WRITE (RNGMODE(0),    0x76);
	REG_WRITE (RNGROEN(0, 0), 0x3F);
	REG_WRITE (RNGROEN(0, 1), 0x06);
	REG_WRITE (RNGROEN(0, 2), 0x00);

	/*	Init reseed counter to 100K */
	return tip_drbg_config(100000);
...
#ifdef LMS_ENABLE
			pROM_NCL_LMS_VerifyData           = TIP_ROM_A3_ADDR_NCL_LMS_VerifyData;
			pROM_NCL_LMS_InitContext          = TIP_ROM_A3_ADDR_NCL_LMS_InitContext;
			pROM_NCL_LMS_Init                 = TIP_ROM_A3_ADDR_NCL_LMS_Init;
			pROM_NCL_LMS_FinalizeContext      = TIP_ROM_A3_ADDR_NCL_LMS_FinalizeContext;
			pROM_NCL_LMS_Clear                = TIP_ROM_A3_ADDR_NCL_LMS_Clear;
#endif
			break;
		default:
			return -1;
 ...
 *  @brief        measure hash of signed area of image
 *
 ...
 *  @param        hashAlg - hash Algorithm to use for verification (SHA2-384 or SHA2-512).
 *  @return       0 if pass or an error code
 */
int tip_image_hash (const struct hash_engine *hash_engine,
							   HEADER_GENERAL_T *imgHeaderInRAM,
							   uint8_t *imageAddr,
							   uint32_t imageLen,
							   uint8_t *hash,
							   uint8_t hashAlg)
{
	int status;
	size_t hash_length = (hashAlg == HASH_TYPE_SHA384) ? SHA384_HASH_LENGTH : SHA512_HASH_LENGTH;
	...
	/* calc SHA on any area that is not DRAM first 16MB */
	if (((uint32_t) imageAddr < TIP_DRAM_WIN0_BASE_ADDR) ||
		((uint32_t) imageAddr >= RAM2_BASE_ADDR)) {
		if (hashAlg == HASH_TYPE_SHA384) {
			status = hash_engine->calculate_sha384 (hash_engine, imageAddr, imageLen, hash, hash_length);
		} else {
#ifdef HASH_ENABLE_SHA512
			status = hash_engine->calculate_sha512 (hash_engine, imageAddr, imageLen, hash, hash_length);
#else
		...
		if (hashAlg == HASH_TYPE_SHA384) {
			status = hash_engine->start_sha384 (hash_engine);
		} else {
#ifdef HASH_ENABLE_SHA512
			status = hash_engine->start_sha512 (hash_engine);
#else
			status = HASH_ENGINE_UNSUPPORTED_HASH;
#endif
		}
		if (status != 0) {
			platform_printf (KRED "hash failed: status = %d\n" KNRM, status);
			return ECC_ENGINE_VERIFY_FAILED;
		}
		size = MIN(imageLen, _64KB_);
		if ((dst_addr % _64KB_) != 0) {
			size = MIN(imageLen, (_64KB_ - (dst_addr % _64KB_)));
		}
		status = hash_engine->update (hash_engine, (uint8_t *) dst_addr, size);
		if (status != 0) {
		...
		while (src_cnt < imageLen) {
			...
			status = hash_engine->update (hash_engine, (uint8_t *) TIP_DRAM_WIN1_BASE_ADDR, size);
			if (status != 0) {
				hash_engine->cancel (hash_engine);
		...
		status = hash_engine->finish (hash_engine, hash, hash_length);
		REG_WRITE (DRAM1WINA, BUILD_FIELD_VAL (DRAM1WINA_WINEN, 1) | block_first);
	}
...
int tip_image_auth_LMS (HEADER_GENERAL_T *imgHeaderInRAM,
							  uint8_t *imageAddr,
							  uint32_t imageLen,
							  uint8_t *key,
							  uint8_t *hash,
							  ...
							  uint8_t hashAlg)
{
	uint8_t lmsCtx[NCL_ECC_CONTEXT_BYTE_SIZE];	__attribute__ ((aligned (16)))
	volatile NCL_STATUS_T nclStat_ret;
	volatile NCL_STATUS_T nclStat;
	int status = ECC_ENGINE_VERIFY_FAILED;

	*sigOK = SECURED_FALSE;
	/* locate the signaure in the footer of the image */
	uint8_t *sig_arr = (uint8_t *) ((uint32_t) imageAddr + imageLen);
	uint32_t chip_version = CHIP_Get_Version ();

	if (chip_version <= ARBEL_VERSION_A2) {
		status = LMS_VerifySig (key, hash, hashAlg, sig_arr, LMS_SIG_SIZE);
	...
	else if (chip_version == ARBEL_VERSION_A3)
	{
		pROM_NCL_LMS_InitContext (lmsCtx);
		pROM_NCL_LMS_Init (lmsCtx);
		nclStat_ret = pROM_NCL_LMS_VerifyData (lmsCtx, key, (NCL_STATUS_T*)&nclStat, hash, hashAlg,  sig_arr, LMS_SIG_SIZE); 
		...
		if (nclStat_ret == NCL_STATUS_OK) {
			if (nclStat == NCL_STATUS_OK) {
				...
				*sigOK = SECURED_FALSE;
				platform_printf_dbg ("NCL_LMS_VerifyData ret %#010lx stat %#010lx\n", nclStat_ret,
								 nclStat);
			}
		} else {
			*sigOK = SECURED_FALSE;
		...
		pROM_NCL_LMS_FinalizeContext (lmsCtx);
		pROM_NCL_LMS_Clear(lmsCtx);
	}
 ...
 * @brief
 *             Performs a security (authentication) check on the specified image using a spesific key */
int tip_image_auth_ECC (const struct ecc_hw *ecc_engine,
							  HEADER_GENERAL_T *imgHeaderInRAM,
							  uint8_t *imageAddr,
							  uint32_t imageLen,
							  uint8_t *key,
							  ...
							  uint8_t hashAlg)
{
	...
	struct ecc_ecdsa_signature sig = { 0 };
	size_t digest_length = (hashAlg == HASH_TYPE_SHA384) ? SHA384_HASH_LENGTH : SHA512_HASH_LENGTH;
	int status = ECC_ENGINE_VERIFY_FAILED;

	*sigOK = SECURED_FALSE;

	platform_printf ("\trSig %#010lx-%#010lx-%#010lx-...\n",
					 ((uint32_t *) imgHeaderInRAM->header.signature)[0],
					 ((uint32_t *) imgHeaderInRAM->header.signature)[1],
					 ((uint32_t *) imgHeaderInRAM->header.signature)[2]);
	platform_printf ("\tsSig %#010lx-%#010lx-%#010lx-...\n",
					 ((uint32_t *) (imgHeaderInRAM->header.signature + ECC_KEY_LENGTH_384))[0],
					 ((uint32_t *) (imgHeaderInRAM->header.signature + ECC_KEY_LENGTH_384))[1],
					 ((uint32_t *) (imgHeaderInRAM->header.signature + ECC_KEY_LENGTH_384))[2]);
	...
	/* Package the public key. The OTP key is stored in NCL little-endian byte order; the ecc_hw
	 * API expects big-endian, so reverse the byte order for both coordinates. */
	pub_key.key_length = ECC_KEY_LENGTH_384;
	tip_mem_swap_endianness (key, pub_key.x, ECC_KEY_LENGTH_384);
	tip_mem_swap_endianness (key + ECC_KEY_LENGTH_384, pub_key.y, ECC_KEY_LENGTH_384);

	/* Package the signature. The image header signature is also in NCL little-endian byte order. */
	sig.length = ECC_KEY_LENGTH_384;
	tip_mem_swap_endianness (imgHeaderInRAM->header.signature, sig.r, ECC_KEY_LENGTH_384);
	tip_mem_swap_endianness (imgHeaderInRAM->header.signature + ECC_KEY_LENGTH_384, sig.s,
		ECC_KEY_LENGTH_384);

	status = ecc_engine->ecdsa_verify (ecc_engine, &pub_key, &sig, hash, digest_length);
	...
	if (status == 0) {
		...
		*sigOK = SECURED_FALSE;
		platform_printf_dbg ("ECDSAVerify failed status %#010lx\n", status);
	}
...
/**
 *  @function     tip_drbg_config
 *  @brief        This function modified the reseed interval using the context from Tip Rom code
 *
 *  @param        reseed interval
 *  @return       0 if pass or an error code
 */
int tip_drbg_config(int reseedInterval)
{
	platform_printf ("tip: ncl_drbg_config is updating to reseed interval %d\n", reseedInterval);

	return pROM_NCL_DRBG_Config (NCL_DRBG_context_l, reseedInterval, FALSE);
}
