...
#define LMS_SIG_SIZE          LMS_SIG_SIZE_H_20_W_8
	...
	uint8_t *digest_out, size_t digest_length)
{
	...
	if (sigOK == SECURED_TRUE) {
		...
		if (b_enc == true) {
			status =
				tip_firmware_component_aes (fw, (struct tip_flash_master_virtual *) &virtual_flash,
					TIP_PER_DEVICE_DECRYPT_KEY, b_enc);
			if (status != 0) {
		...
		platform_printf (KRED "ECC %s key %d, SIG FAIL" NEWLINE KNRM, fw_name, fw->key_index);
		return FIRMWARE_COMPONENT_BAD_SIGNATURE;
	}

#ifdef LMS_ENABLE
	/* after verification with ECC key, moving to verification with LMS key */
	if (header->header.enableLMS == 0x01) {
		/* LMS key index hard coded in the meantime */
		key_index = 2;
		platform_printf (KCYN "%s is signed with LMS key %d" NEWLINE KNRM, fw_name, key_index);
		
		/* locate the LMS key in SKMT */
		status = tip_firmware_component_get_key (fw, kmt, skmt, key_index, &pub_key, &pub_key_len);
		if (status != 0) {
			platform_printf (KRED "failed tip_firmware_component_get_key" NEWLINE KNRM);
			return FIRMWARE_COMPONENT_VERIFY_FAILED;
		}
		
		/* locate the signaure in the footer of the image */
		sig_arr = (uint8_t *) ((uint32_t) imageAddr + imageLen);
	
		/* perform LMS verification */
		status = LMS_VerifySig (pub_key, imageAddr, imageLen, sig_arr, LMS_SIG_SIZE_H_20_W_8);
		if (status != DEFS_STATUS_OK) {
			platform_printf (KRED "LMS Verify with key %d failed" NEWLINE KNRM, key_index);
			return FIRMWARE_COMPONENT_BAD_SIGNATURE;
		}
	}
#endif
...
int tip_firmware_component_load_to_memory (struct tip_firmware_component *fw)
{
	...
	if (fw->header_ram->header.enableLMS == 0x01) {
		tip_memcpy (dst_addr + img_size, src_flash_addr + img_size, LMS_SIG_SIZE_H_20_W_8, dram_window,
			true);
	}
