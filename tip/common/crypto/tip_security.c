...
#define TIP_ROM_Z1_ADDR_NCL_SHA_Calc                     ((ROM_NCL_SHA_Calc_A1)(0x00009c3c + THUMB_BIT))
...
int tip_Security_Sha_Calc(void* context,NCL_SHA_TYPE_T shaType, 
								const uint8_t *data, uint32_t dataLen, uint8_t* hashDigest)
{
	...
	switch (chip_version) {
		...
		case ARBEL_VERSION_A2:
			return pROM_NCL_SHA_Calc_A1(context, shaType, data, dataLen, hashDigest);
			break;
...
int tip_init_rom_ncl (void)
{
	...
	switch (chip_version) {
		case ARBEL_VERSION_Z1:			
			...
			pROM_NCL_SHA_Calc_A1                 = TIP_ROM_Z1_ADDR_NCL_SHA_Calc;
							   ...
							   uint8_t *hash)
{
	...
	if (((uint32_t) imageAddr < TIP_DRAM_WIN0_BASE_ADDR) ||
		...
		nclStat_ret =
			tip_Security_Sha_Calc(shaCtx, NCL_SHA_TYPE_2_512, imageAddr, imageLen, (uint8_t *) hash);
