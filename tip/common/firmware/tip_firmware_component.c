...
extern ROM_NCL_AES_CCM_Decrypt            pROM_NCL_AES_CCM_Decrypt;
extern ROM_NCL_AES_CCM_Encrypt            pROM_NCL_AES_CCM_Encrypt;
extern ROM_NCL_AES_CCM_SetParams          pROM_NCL_AES_CCM_SetParams;
extern ROM_NCL_AES_Clear                  pROM_NCL_AES_Clear;
extern ROM_NCL_AES_ClearParams            pROM_NCL_AES_ClearParams;
extern ROM_NCL_AES_CMAC_Generate          pROM_NCL_AES_CMAC_Generate;
extern ROM_NCL_AES_CMAC_Verify            pROM_NCL_AES_CMAC_Verify;
...
extern ROM_NCL_AES_GetContextSize         pROM_NCL_AES_GetContextSize;
extern ROM_NCL_AES_Init                   pROM_NCL_AES_Init;
extern ROM_NCL_AES_InitContext            pROM_NCL_AES_InitContext;
extern ROM_NCL_AES_LockKey                pROM_NCL_AES_LockKey;
extern ROM_NCL_AES_Power                  pROM_NCL_AES_Power;
extern ROM_NCL_AES_SelectKey              pROM_NCL_AES_SelectKey;
extern ROM_NCL_AES_SetIv                  pROM_NCL_AES_SetIv;
extern ROM_NCL_AES_SetKey                 pROM_NCL_AES_SetKey;
extern ROM_NCL_AES_SetParams              pROM_NCL_AES_SetParams;
extern ROM_NCL_AES_SetSeed                pROM_NCL_AES_SetSeed;
extern ROM_NCL_AES_Update                 pROM_NCL_AES_Update;
...
#ifdef _PLATFORM_RTOS_  /* only L1 has a virtual flash. Only L1 is handling recovery image verification */
/**
 * Handler for the recovery flash.
 */
extern struct spi_flash *recovery_flash;
	...
	uint8_t **pub_key, size_t *pub_key_length)
{
	...
	if (fw->img_type == IMG_KMT) {
		...
		if (valid_otp[0] != 0xAA) {
			platform_printf ("OTP key %d is invalid" NEWLINE, ind);
		...
		*pub_key = key_otp;
		...
		if (fw->header_ram) {
			...
			*pub_key = (uint8_t *) kmt->fw->header_ram + sizeof (IMG_HEADER_T) + ind * 128;
		} else {
			*pub_key = (uint8_t *) kmt->fw->header_flash + sizeof (IMG_HEADER_T) + ind * 128;
		...
		status = tip_skmt_key_lookup (skmt, ind, pub_key, pub_key_length);
		if (status != 0) {
			platform_printf ("SKMT key %d not found" NEWLINE KNRM, ind);
			return status;
		}
	}
	platform_printf ("public key %d is 0x%x-0x%x-0x%x-..." NEWLINE, ind, (*pub_key)[0],
		(*pub_key)[1], (*pub_key)[2]);
...
static int tip_firmware_component_aes (struct tip_firmware_component *fw, 
	struct tip_flash_master_virtual *flash, struct tip_aes_ncl_engine *aes_engine,
	uint8_t key_ind, bool b_enc)
{
	TIP_HEADER_STRUCT_T *tip_rom_header;
	HEADER_GENERAL_T *tip_rom_header2;
	uint8_t aes_context[NCL_AES_CONTEXT_BYTE_SIZE];
	uint32_t *data_src;
	uint32_t *data_dst;
	uint32_t fw_len;
	...
	UNUSED (aes_engine); /* using CBC only. Cerberus engines support GCM only */
	...
	/* check if encrypt bit is set in header */
	if (tip_rom_header->tft_enc_ctl & 0x01) {
		...
		platform_printf ("%s AES %s addr %010lx src %010lx dst %010lx len %010lx" NEWLINE,
			tip_firmware_component_get_name (fw->img_type), (b_enc == true) ? "ENC" : "DEC",
			(uint32_t) tip_rom_header, (uint32_t) data_src, data_dst, fw_len);

		FUSE_WRPR_get (TIP_AES_KEY0_VALID_PROPERTY, aes_context);
		if (aes_context[0] != 0xAA) {
			platform_printf (KRED "Missing AES key on device" KNRM NEWLINE);
			return AES_ENGINE_NO_KEY;
		}

		status = pROM_NCL_AES_InitContext (aes_context);
		if (status != NCL_STATUS_OK) {
			return AES_ENGINE_NO_MEMORY;
		}

		status = pROM_NCL_AES_Init (aes_context);
		if (status != NCL_STATUS_OK) {
			return AES_ENGINE_HW_NOT_INIT;
		}

		status = pROM_NCL_AES_SelectKey (aes_context, key_ind);
		if (status != NCL_STATUS_OK) {
			return AES_ENGINE_SET_KEY_FAILED;
		}

		status = pROM_NCL_AES_SetParams (aes_context, NULL, NCL_AES_KEY_SIZE_256, NCL_AES_MODE_CBC,
			iv);
		if (status != NCL_STATUS_OK) {
		...
		if (b_enc == false) {
			status = pROM_NCL_AES_Decrypt (aes_context, data_src, data_dst, fw_len);
			if (status != NCL_STATUS_OK) {
				return AES_ENGINE_DECRYPT_FAILED;
			}
		} else {
			status = pROM_NCL_AES_Encrypt (aes_context, data_src, data_dst, fw_len);
			if (status != NCL_STATUS_OK) {
				return AES_ENGINE_DECRYPT_FAILED;
			}
		}

		pROM_NCL_AES_FinalizeContext (aes_context);
	}
	...
	uint8_t *digest_out, size_t digest_length)
{
	...
	/* check if the image in on the virtual flash. If it does, TIP is doing update, so need to
	 * encrypt */
	if (&virtual_flash == (struct spi_flash *) fw->base.flash) {
		b_enc = true;
	}
#endif
	...
	if (b_enc == false) {
		status = tip_firmware_component_aes (fw, (struct tip_flash_master_virtual *) &virtual_flash, NULL,
	...
	if (header->header.key_mask_select != 0) {
		key_index_min = 0;
		key_index_max = tip_firmware_component_get_key_count (fw, kmt, skmt);
		key_mask = header->header.key_mask_select;
	} else {
		key_index = header->header.KeyIndex;
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		status = tip_firmware_component_get_key (fw, kmt, skmt, key_index, &pub_key, &pub_key_len);
		if (status != 0) {
			continue;
		}
		...
		if (hal_status != DEFS_STATUS_OK) {
			platform_printf (KRED"Verify with key %d failed" NEWLINE KNRM, key_index);
		}

		if (sigOK == SECURED_TRUE) {
			platform_printf (KGRN "Image verified with key %d" NEWLINE KNRM, key_index);
			break;
		} else {
			platform_printf ("Image verify fail with key %d" NEWLINE KNRM, key_index);
		}
	}
	...
	if (sigOK == SECURED_TRUE) {
		...
		/* check if need to enc\dec image. During boot: decrypt, during update : encrypt */
#ifdef _PLATFORM_RTOS_
		/* check if the image in on the virtual flash. If it does,
		 * TIP is doing update, so need to encrypt
		 */
		if (&virtual_flash == (struct spi_flash *) fw->base.flash) {
			b_enc = true;
		...
		return tip_version_set (tip_version, version, next_version, fw->img_type);
	}
...
int tip_firmware_component_load_to_memory (struct tip_firmware_component *fw)
{
	...
	if ((img_size < sizeof (HEADER_GENERAL_T)) || (img_size > size_max)) {
		platform_printf (KRED
			"invalid image size %d, fw->header_flash->header.codeSize is %d + sizeof (HEADER_GENERAL_T) %d  " NEWLINE
				KNRM,
			img_size, fw->header_flash->header.codeSize, sizeof (HEADER_GENERAL_T));
		return -1;
	}
...
int tip_firmware_component_load_external_reg_table (struct tip_firmware_component *fw, uint16_t reset)
{
	...
	regs_offset = (uint32_t)fw->header_ram + fw->header_ram->header.regs_offset + sizeof(HEADER_GENERAL_T);

	reg_cnt_total = (fw->header_ram->header.codeSize - fw->header_ram->header.regs_offset) / reg_line_size;

	platform_printf(KMAG "%s reg table found %#010lx reset %#05x %d regs" NEWLINE KNRM, 
		tip_firmware_component_get_name (fw->img_type), regs_offset, reset, reg_cnt_total);
		...
		switch (reg_size) {
			case 0x02: 
				if (reg_mask == 0xFFFFFFFF) {
					...
					*(uint32_t *)reg_addr = (*(uint32_t *)reg_addr & ~reg_mask) |
						(reg_value & reg_mask);
				}
				
				platform_printf ("ind %d addr %#010lx value %#010lx mask %#010lx reset %#05x "
						 "delay %#010lx us before %#010lx after %#010lx" NEWLINE,
						 reg_cnt, reg_addr, reg_value, reg_mask, reg_reset, reg_delay_us,
						 reg_before, *(uint32_t *)reg_addr);
				break;
			case 0x01:
				reg_mask &= 0x0000FFFF;
				...
				platform_printf ("ind %d addr %#010lx value %#06x mask %#06x reset %#05x "
						 "delay %#010lx us before %#06x after %#06x" NEWLINE,
						 reg_cnt, reg_addr, reg_value, reg_mask, reg_reset, reg_delay_us,
						 reg_before, *(uint16_t *)reg_addr);
				break;
			case 0x00:
			default:
				reg_mask &= 0x000000FF;
				...
				platform_printf ("ind %d addr %#010lx value %#04x mask %#04x reset %#05x "
						 "delay %#010lx us before %#04x after %#04x" NEWLINE,
						 reg_cnt, reg_addr, reg_value, reg_mask, reg_reset, reg_delay_us,
						 reg_before, *(uint8_t *)reg_addr);
				break;
		}
