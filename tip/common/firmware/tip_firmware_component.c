	...
	uint8_t *imageAddr, uint32_t imageLen)
{
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		/* perform LMS verification */
			...
			status = LMS_VerifySig (pub_key, digest_out, SHA512_HASH_LENGTH, sig_arr, LMS_SIG_SIZE);
...
int tip_firmware_component_load_to_memory (struct tip_firmware_component *fw)
{
	...
	uint32_t fustrap1;
	FUSE_WRPR_get (FUSTRAP1_PROPERTY, (uint8_t *) &fustrap1);
	if (fw->header_ram->header.enableLMS == 0x01 || READ_VAR_FIELD (fustrap1, FUSTRAP1_oLMS_AUTH_EN)) {
