							  ...
							  SECURED_BOOLEAN_T *sigOK)
{
	...
	*sigOK = SECURED_FALSE;
	...
	if (chip_version <= ARBEL_VERSION_A2) {
		status = LMS_VerifySig (key, hash, SHA512_HASH_LENGTH, sig_arr, LMS_SIG_SIZE);
	...
	else if (chip_version == ARBEL_VERSION_A3)
	{
		...
		nclStat_ret = pROM_NCL_LMS_VerifyData (lmsCtx, key, (NCL_STATUS_T*)&nclStat, hash, SHA512_HASH_LENGTH,  sig_arr, LMS_SIG_SIZE); 
