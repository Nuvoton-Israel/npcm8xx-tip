...
#ifdef BMC_DIRECT
static void tip_bmc_direct_finalyze_command (int status)
{
...
void NVIC_BMC_direct_handler (uint16_t num)
{
	...
	if ((notification & BMC_DIRECT_NOTIFCATION) > 0) {
		...
		if ((cmd > BMC_RESET_0) && (cmd < BMC_MAX_COMMANDS)) {
			...
			tip_bmc_direct_finalyze_command (CMD_CHANNEL_INVALID_ARGUMENT);
		}
	}
		...
		/* handle post boot BMC reset event */
			...
			switch (notification) {
				...
				case BMC_DIRECT_COMMAND_FL_PROG: {
					...
					tip_bmc_direct_finalyze_command (status);
					break;
				}
