	...
	uint8_t **pub_key, int pub_key_length, bool is_ECC_key, bool is_LMS_key)
{
	...
	/* Handle KMT keys */
		...
		if (is_ECC_key) {
			...
			offset_to_add_in_KMT =
				(kmt_keys_per_algo * ECC_KEY_SIZE) + ((ind - kmt_keys_per_algo) * LMS_KEY_SIZE);
			index_to_read_key = 8;
		}
#endif
		if (fw->header_ram) {
	...
	platform_printf (KRED "key is valid but empty" NEWLINE KNRM, ind);
	return KEY_MANIFEST_UNSUPPORTED_KEY;
}
...
static int tip_firmware_component_verify_ecc_or_lms_sig (struct tip_firmware_component *fw,
	...
	uint8_t *imageAddr, uint32_t imageLen, uint8_t *digest_buf)
{
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		if (isECC) {
			status = tip_image_auth_ECC (header, imageAddr, imageLen, pub_key, digest_buf, &sigOK);
			...
			if (status != DEFS_STATUS_OK) {
				platform_printf (KRED "Verify with key %d failed, status is %x" NEWLINE KNRM,
					key_index, status);
			}

			if (sigOK == SECURED_TRUE) {
				platform_printf (KGRN "Image verified with ECC key %d " NEWLINE KNRM, key_index);
				break;
			}
			else {
				platform_printf ("Image verify fail with ECC key %d" NEWLINE KNRM, key_index);
			}
		}
#ifdef LMS_ENABLE
		/* perform LMS verification */
			...
			/* TODO on A3: use pROM_NCL_LMS_VerifyData, return NCL status properly */
	...
	if (sigOK == SECURED_TRUE) {
		...
		platform_printf (KRED "%s key %d %s SIG FAIL" NEWLINE KNRM, fw_name, fw->key_index, name);
	...
	uint8_t *digest_out, size_t digest_length)
{
	...
	status = tip_firmware_component_verify_ecc_or_lms_sig (fw, kmt, skmt, digest_out, digest_length, true,
		false, header, imageAddr, imageLen, digest_buf);
...
#ifdef LMS_ENABLE
	{
		...
		if (header->header.enableLMS == 0x01 ||
			(ChipVersion == ARBEL_VERSION_A3 && READ_VAR_FIELD (fustrap1, FUSTRAP1_oLMS_AUTH_EN))) {
			if (!(fw->img_type == IMG_KMT && ChipVersion == ARBEL_VERSION_A2)) {
				/* ignore A2 verification of KMT with LMS OTP key since doesn't exist */
				status = tip_firmware_component_verify_ecc_or_lms_sig (fw, kmt, skmt, digest_out,
					digest_length, false, true, header, imageAddr, imageLen, digest_buf);
...
int tip_firmware_component_load_to_memory (struct tip_firmware_component *fw)
{
	...
	if (fw->header_ram->header.enableLMS == 0x01) {
		tip_memcpy (dst_addr + img_size, src_flash_addr + img_size, LMS_SIG_SIZE_H_20_W_8,
			dram_window, true);
	}
#endif
