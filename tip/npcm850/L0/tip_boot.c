...
/**
 * Shared NCL state for L0 (bare-metal, no threading).
 */
static struct tip_rom_ncl_shared_state_boot ncl_shared_state_impl;
...
/**
 * Variable context system hash engine
 */
static struct tip_hash_ncl_engine_state system_hash_context;
...
/**
 * Variable context for HW ECC engine
 */
static struct tip_ecc_hw_ncl_state hw_ecc_context;
...
static int load_main_application (struct spi_flash *flash, uint16_t reset)
{
	...
	tip_version_set (tip_version, fw.header_flash->header.version,
		fw.header_flash->header.next_version, IMG_KMT);
	...
	tip_version_set (tip_version,  fw.header_flash->header.version,
		fw.header_flash->header.next_version , IMG_TFT_L0);
	...
	status = tip_firmware_component_verify (&skmt_img, &system_hash.base, &tip_kmt, NULL,
		attestation->skmt_hash, sizeof (attestation->skmt_hash), &hw_ecc.base);
	...
	tip_L1_system_control_init (&tip_L1_sys_ctrl, fw.header_ram->header.SystemControlFlags);

	status = tip_firmware_component_verify (&fw, &system_hash.base, NULL, &tip_skmt,
		attestation->app_hash, sizeof (attestation->app_hash), &hw_ecc.base);
	if (status != 0 && TIP_SECBOOT_IS_ACTIVE ()) {
	...
	if (tip_firmware_component_check_external_reg_table (&fw) == true) {
		status = tip_firmware_component_load_external_reg_table (&fw, reset);
	...
	riot_tcb.svn = (uint8_t*) &tip_version->tip_fw_l0_version;
	riot_tcb.svn_length = sizeof (tip_version->tip_fw_l0_version);
	...
	alias_tcb.svn = (uint8_t*) &tip_version->tip_fw_l1_version;
	alias_tcb.svn_length = sizeof (tip_version->tip_fw_l1_version);
	alias_tcb.fwid = x509_alias_tcb_fwid;
	alias_tcb.fwid_hash = HASH_TYPE_SHA384;

	status = riot.base.base.generate_alias_key (&riot.base.base, alias_tcb.fwid,
		alias_tcb.fwid_hash);
...
int main (void)
{
	...
	status = tip_rom_ncl_shared_state_boot_init (&ncl_shared_state_impl);
	if (status != 0) {
		return status;
	}
	...
	status = ecc_ecc_hw_init (&ecc, &hw_ecc.base, NULL);
	if (status != 0) {
	...
	REG_WRITE (TIP_INVALIDATE_OTP_KEYS_SCR, key_invalid);
