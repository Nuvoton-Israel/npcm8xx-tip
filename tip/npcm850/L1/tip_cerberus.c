 ...
 */
static struct attestation_requester system_attestation_requester;
 ...
 */
static platform_timer routing_table_rq_timer;
#endif
...
/* TODO: Manifest root key should be obtained from SKMT via key manifest API.
   Now root key is a static one generated via OpenSSL for testing. */
/**
...
static struct ecc_der_public_key manifest_root_key = {
	.der = {
		0x30,0x76,0x30,0x10,0x06,0x07,0x2a,0x86,0x48,0xce,0x3d,0x02,0x01,0x06,0x05,0x2b,
		0x81,0x04,0x00,0x22,0x03,0x62,0x00,0x04,0xcc,0x1e,0xab,0xa4,0x47,0xf5,0x6c,0xcc,
		0xc5,0x9e,0x28,0x9b,0xfc,0xbc,0x33,0x3d,0xee,0x6a,0xda,0x10,0x76,0xe0,0x65,0xdd,
		0x5d,0x79,0xcf,0x95,0xb9,0x62,0x82,0x09,0xbc,0xf7,0x28,0xa1,0xea,0xb6,0xc5,0x25,
		0x93,0x86,0x84,0x74,0x81,0xb8,0x23,0x39,0x32,0x20,0x8c,0x38,0x45,0xea,0x8d,0x12,
		0x15,0xe1,0x32,0x37,0x6b,0x38,0x2e,0xc8,0x5c,0xbd,0x63,0xf1,0xb0,0x2d,0xf6,0x4b,
		0x4d,0xb5,0x4e,0xe0,0x1f,0x10,0xdc,0x8b,0x27,0xcc,0x90,0xcb,0xe3,0x31,0xea,0x6c,
		0x52,0xd4,0x08,0x8a,0x93,0x14,0x19,0x5f
	},
	.length = 120
};
 ...
 */
static bool has_active_pcd;

/**
 * Flag indicating if there is an active PCD with at least one component configured.
 */
static bool pcd_has_components;

/**
 * Maximum duration to wait for MCTP bridge to assign Cerberus an EID before attempting to get
 * routing table.
 */
static uint32_t get_routing_table_timeout_ms;
...
static int init_cmd_interface ()
{
	...
	struct pcd_rot_info rot_info = {
		...
		.eid = MCTP_BASE_PROTOCOL_PA_ROT_CTRL_EID,
		.bridge_i2c_addr = 0,
...
static int initialize_riot_manager ()
{
	...
	status = flash_store_contiguous_blocks_init_variable_storage_decreasing (&keystore_flash,
		&keystore_flash_context, &main_flash->base, KEY_STORE_ADDR(main_flash), MAIN_KEYSTORE_MAX_KEYS, 0,
		&shared_hash.base);
	if (status != 0) {
		return status;
	}
...
#ifdef CERBERUS_ENABLE_COMPONENT_ATTESTATION
	status = system_state_manager_init (&system_state, &main_flash->base, SYSTEM_STATE_ADDR(main_flash));
	if (status != 0) {
		return status;
	}
	platform_printf(KGRN "TIP system at flash0 %#010lx" NEWLINE KNRM,  SYSTEM_STATE_ADDR(main_flash));
#endif
...
static int initialize_pcd_management ()
{
	...
	status = pcd_flash_init (&pcd_region1, &main_flash->base, &shared_hash.base,
		pcd_region1_addr(main_flash), pcd_region1_signature, sizeof (pcd_region1_signature),
		pcd_region1_platform_id, sizeof (pcd_region1_platform_id));
	if (status != 0) {
		return status;
	}

	status = pcd_flash_init (&pcd_region2, &main_flash->base, &shared_hash.base,
		pcd_region2_addr(main_flash), pcd_region2_signature, sizeof (pcd_region2_signature),
		pcd_region2_platform_id, sizeof (pcd_region2_platform_id));
	if (status != 0) {
		return status;
	}
	...
	platform_printf (KGRN "PCD: active=0x%p (%#010lx,%#010lx)" NEWLINE KNRM, active_pcd,
		pcd_region1_addr(main_flash), pcd_region2_addr(main_flash));
	...
	if (ROT_IS_ERROR(pcd_region1.base.base.is_empty(&pcd_region1.base.base))) {
		platform_printf ("pcd1 invalid. Format %#010lx" NEWLINE, pcd_region1_addr(main_flash));
		pcd_region1.base_flash.flash->block_erase (pcd_region1.base_flash.flash,
			pcd_region1_addr(main_flash));
	}

	if (ROT_IS_ERROR(pcd_region2.base.base.is_empty(&pcd_region2.base.base))) {
		platform_printf ("pcd2 invalid. Format %#010lx" NEWLINE, pcd_region2_addr(main_flash));
		pcd_region2.base_flash.flash->block_erase (pcd_region2.base_flash.flash,
			pcd_region2_addr(main_flash));
	}
...
static int initialize_component_management ()
{
	...
	status = cfm_flash_init (&cfm_region1, &main_flash->base, &shared_hash.base,
		cfm_region_addr1 (main_flash), cfm_region1_signature, sizeof (cfm_region1_signature),
		cfm_region1_platform_id, sizeof (cfm_region1_platform_id));
	if (status != 0) {
		return status;
	}

	status = cfm_flash_init (&cfm_region2, &main_flash->base, &shared_hash.base,
		cfm_region_addr2 (main_flash), cfm_region2_signature, sizeof (cfm_region2_signature),
		cfm_region2_platform_id, sizeof (cfm_region2_platform_id));
	if (status != 0) {
		return status;
	}
	...
	status = cfm_manager_flash_init (&component_manifest, &cfm_region1, &cfm_region2,
		&system_state, &shared_hash.base, &cfm_verification.base_verify);
	if (status != 0) {
		return status;
	}
	...
	if (ROT_IS_ERROR(cfm_region1.base.base.is_empty(&cfm_region1.base.base))) {
		platform_printf ("CFM1 invalid. Format %#010lx" NEWLINE, cfm_region_addr1(main_flash));
		cfm_region1.base_flash.flash->block_erase (cfm_region1.base_flash.flash,
			cfm_region_addr1(main_flash));
	}

	if (ROT_IS_ERROR(cfm_region2.base.base.is_empty(&cfm_region2.base.base))) {
		platform_printf ("CFM2 invalid. Format %#010lx" NEWLINE, cfm_region_addr2(main_flash));
		cfm_region2.base_flash.flash->block_erase (cfm_region2.base_flash.flash,
			cfm_region_addr2(main_flash));
	}
	...
	platform_printf (KGRN "CFM: active=0x%p, pending=0x%p (%#010lx,%#010lx)" NEWLINE KNRM, 
		active, pending, cfm_region_addr1(main_flash), cfm_region_addr2(main_flash));
...
static void cerberus_init (void *unused)
{
	...
	platform_printf(KMAG "Deubg log at recovery flash offset %#010lx" NEWLINE KNRM, DEBUG_LOG_FLASH_ADDR(recovery_flash));
	...
	status = bmc_task_init (&bmc_reset_task, CERBERUS_PRIORITY_BACKGROUND, 256 * 4);
	if (status != 0) {
