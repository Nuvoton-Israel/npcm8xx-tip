...
/*---------------------------------------------------------------------------------------------------------*/
	...
	//serial_printf("in SHA_Start , handlePtr is %x\n", handlePtr);


void SHA256_Init (SHA256_CTX *ctx)
{
#ifdef SHA_CAPABILITY_MULTIPLE_MODULES


    (void)SHA_Config(ctx, SHA_MODULE_0, FALSE, TRUE, NULL);
#endif

    (void)SHA_Init(ctx);

    (void)SHA_Start(ctx, SHA_TYPE_SHA256);
}

void SHA256_Update (SHA256_CTX *ctx, const void *src, unsigned int count)
{
	
    (void)SHA_Update(ctx, (const UINT8*)src, count);
}

void SHA256_Final (unsigned char *digest, SHA256_CTX *ctx)
{
    (void)SHA_Finish(ctx, digest);
}
#endif
