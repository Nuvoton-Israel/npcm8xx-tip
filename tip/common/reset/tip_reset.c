...
#ifdef GPIO_WOL
	if (GPIO_TIP_GetInterruptStatus (TIP_WOL_GPIO) == 1) {
...
{
	uint8_t wd1_count = REG_READ (TIP_SCR3) & 0xFF;
	uint8_t wol_count = (REG_READ (TIP_SCR3) & 0xFF00) >> 8U;
	...
	struct system *system)
{
	...
	if ((MAX_RESET_COUNT_WD1 != 0) && (tip_scr1 & RESET_WD1)) {
		...
		if (reset_counters_l[RST_WD1] < MAX_RESET_COUNT_WD1) {
			reg_cache = REG_READ (TIP_SCR3) & ~(0xFF);
	...
	if (GPIO_TIP_GetInterruptStatus (TIP_WOL_GPIO) == 1) {
		...
		if ((MAX_RESET_COUNT_WOL != 0) && (reset_counters_l[RST_WOL] < MAX_RESET_COUNT_WOL)) {
			reg_cache = REG_READ (TIP_SCR3) & ~(0xFF00);
			reg_cache |= (reset_counters_l[RST_WOL] << 8U);
			REG_WRITE (TIP_SCR3, reg_cache);
		}
		else {
			reset_maxed_out = true;
...
void tip_update_reset_indication (bool updateIntcr2)
{
	...
	SET_REG_FIELD (TIP_SCR1, TIP_RST_INDICATION_RESSR_23_31, 0);
	SET_REG_FIELD (TIP_SCR1, TIP_RST_INDICATION_WDRST_STS, 0);
	SET_REG_FIELD (TIP_SCR1, TIP_RST_INDICATION_TIP_SWRST_STS, 0);
	SET_REG_FIELD (TIP_SCR1, TIP_RST_INDICATION_DBGRST_STS, 0);
	SET_REG_FIELD (TIP_SCR1, TIP_RST_INDICATION_SW_SEC_ERR, 0);
#ifdef GPIO_WOL
