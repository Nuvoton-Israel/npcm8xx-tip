...
/**
 * Global AES engine for firmware encryption/decryption
 */
extern struct tip_aes_ncl_engine hw_aes;
	...
	uint8_t **pub_key, int pub_key_length, bool is_ECC_key, bool is_LMS_key)
{
	...
	int max = tip_firmware_component_get_key_count (fw, kmt, skmt);
	if (ind < 0 || ind >= max) {
		platform_printf ("Error: key index %d out of range [0..%d)\n", ind, max);
		return FIRMWARE_COMPONENT_INVALID_ARGUMENT;
	}
...
static int tip_firmware_component_aes (struct tip_firmware_component *fw,
	struct tip_flash_master_virtual *flash, struct tip_aes_ncl_engine *engine, 
	uint8_t key_ind, bool b_enc)
{
	...
	if (tip_rom_header->tft_enc_ctl == 0x03) {
		...
		/* init the engine key and mode */
		ChipVersion = CHIP_Get_Version ();
		aes_mode = (ChipVersion >= ARBEL_VERSION_A3) ? NCL_AES_MODE_CTR : NCL_AES_MODE_CBC;
		status = engine->set_mode (engine, aes_mode);
		if (status != 0) {
			goto aes_done;
		}

		status = engine->select_key (engine, key_ind);
		if (status != 0) {
			goto aes_done;
		}

		if (b_enc == true) {
			status = engine->base.encrypt_data (&(engine->base), (uint8_t*) data_src,
				fw_len, (uint8_t*) iv, NCL_AES_KEY_SIZE_256, (uint8_t*) data_dst, fw_len, NULL,
				0);
		}
		else {
			status = engine->base.decrypt_data (&(engine->base), (uint8_t *) data_src,
				fw_len, NULL, (uint8_t *) iv, NCL_AES_KEY_SIZE_256, (uint8_t *) data_dst, fw_len);
#ifdef LMS_ENABLE
		if (fw->is_lms_sig_exists) {
			/* Signature is after header + image */
			uint32_t sig_src_offset = header_size + fw_len; 
			uint32_t sig_src_addr = (uint32_t)tip_rom_header + sig_src_offset;
			uint32_t sig_dst_addr = TIP_VIRTUAL_FLASH_BASE_ADDR + sig_src_offset;

			platform_printf("AES DEBUG: Copying LMS sig from %#010lx to %#010lx, size=%d" NEWLINE, sig_src_addr,
				sig_dst_addr, LMS_SIG_SIZE);
			tip_memcpy(sig_dst_addr, sig_src_addr, LMS_SIG_SIZE, false, false);
		}
#endif
		}
aes_done:
...
static int tip_firmware_component_verify_ecc_or_lms_sig (struct tip_firmware_component *fw,
	...
	uint8_t *imageAddr, uint32_t imageLen, const struct ecc_hw *ecc)
{
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		if (isECC) {
			status = tip_image_auth_ECC (ecc, header, imageAddr, imageLen, pub_key, digest_out, &sigOK, hash_alg_ecc);
	...
	const struct ecc_hw *ecc)
{
	...
	if (tip_L1_sys_ctrl.tip_support_encryption) {
		...
		if (b_enc == false) {
			status = tip_firmware_component_aes (fw, (struct tip_flash_master_virtual *) &virtual_flash,
				&hw_aes, TIP_PER_DEVICE_DECRYPT_KEY, b_enc);
			if (status != 0) {
	...
	hal_status = tip_image_hash (hash, header, imageAddr, imageLen, digest_buf, hash_alg);
	 ...
	 * Cap the copy at the caller's buffer size so a SHA-384 caller (48 B)
	 * still receives its digest from the 64 B local scratch buffer. */
	if (digest_out != NULL) {
		memcpy (digest_out, digest_buf,
			(digest_length < sizeof (digest_buf)) ? digest_length : sizeof (digest_buf));
	}

	status = tip_firmware_component_verify_ecc_or_lms_sig (fw, kmt, skmt, digest_buf, digest_length, true,
		false, header, imageAddr, imageLen, ecc);
...
#ifdef LMS_ENABLE
	{
		...
		if (header->header.enableLMS == 0x01 || fustrap1_lms_enabeld ||
			...
			status = tip_firmware_component_verify_ecc_or_lms_sig (fw, kmt, skmt, digest_buf,
				digest_length, false, true, header, imageAddr, imageLen, ecc);
			if (status != 0) {
				return FIRMWARE_COMPONENT_VERIFY_FAILED;
			}
		}
...
int tip_firmware_component_load_to_memory (struct tip_firmware_component *fw)
{
	...
	if ((dst_addr < addr_min) || (dst_addr > addr_max - img_size)) {
