...
static void flush_data_background_task (struct flush_data_background *flush_data)
{
	...
	while (1) {
		...
		/* Sleep timeout has been increased to 30secs to avoid TIP FW accessing the flash before
		 * Linux kernel is loaded into memory.
		 * TODO: Indentify a mechanism to synchronize BMC and TIP flash access instead.
		 */

		platform_msleep (1000000);
