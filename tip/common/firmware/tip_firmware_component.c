	...
	struct tip_primary_key_manifest *kmt, struct tip_secondary_key_manifest *skmt)
{
	...
	switch (fw->img_type) {
		case IMG_KMT:
			return FUSE_WRAPPER_NUM_OF_ECC_KEYS; /* KMT is signed with OTP keys */

		case IMG_TFT_L0:
			return kmt->num_of_kmt_keys; /* L0 signed with a KMT key */

		default:
			return skmt->num_of_keys;
	}
}
	...
	uint8_t key_ind, bool b_enc)
{
	...
	if (tip_rom_header->tft_enc_ctl & 0x01) {
		...
		if ((fw->header_ram == NULL) && (b_enc == false)) {
			...
			fw->header_decrypt_staging = (HEADER_GENERAL_T *) flash->base_addr;
		}
	...
	uint8_t *digest_out, size_t digest_length)
{
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		if ((key_mask & (0x01 << key_index)) == 0) {
			platform_printf ("skip key %d mask %#010lx \n", key_index, key_mask);
			continue;
		}
		...
		if (hal_status != DEFS_STATUS_OK) {
			platform_printf ("Verify with key %d failed" NEWLINE KNRM, key_index);
			status = FIRMWARE_COMPONENT_VERIFY_FAILED;
		}
