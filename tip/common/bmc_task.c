 ...
 * @param combo1_start_addr Address where the combo1 was found.
 * @return combo 1 location on flash if success or an error code.
 */
		...
		struct hash_engine *hash, uint16_t reset_source, uint32_t *combo1_start_addr)
{
	...
	/* return the start address of combo 1 on flash (bootblock)
	 * typically this address would be base + 512KB or 4MB. location selected by IGPS
	 */
	return status;
}
			...
			switch (notification) {
				case BMC_RESET_CMD:
					...
					if (ROT_IS_ERROR (status) == false) {
						...
						platform_printf (KRED "Reload failed with status %#010lx" NEWLINE KNRM, status);
