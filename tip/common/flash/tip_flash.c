...
#define FIU_NUM_OF_MODULES_ACTIVE 2
 ...
 */
static int tip_flash_get_index_of_currently_running_image (uint32_t *spi, uint32_t *fiu,
	uint32_t *cs)
{
	int status;
	...
	platform_printf (KGRN NEWLINE "This image is loaded from spi %d, fiu %d, cs %d offset %d, \
...
int tip_flash_xfer (const struct flash_master *spi, const struct flash_xfer *xfer)
{
	...
	if ((xfer->cmd == FLASH_CMD_READ) ||
		...
		if (read_cmd_default[fiu] != xfer->cmd) {
			FIU_ConfigCommand (fiu, xfer->cmd, 0);
			read_cmd_default[fiu] = xfer->cmd;
			platform_printf (KMAG "read command FIU_DRD_CFG[%d] %#010lx" NEWLINE KNRM, fiu,
				REG_READ (FIU_DRD_CFG (fiu)));
		}
	...
	if((xfer->cmd == FLASH_CMD_4BYTE_4K_ERASE) || 
		...
		platform_printf("TIP ERASE fiu%d cs%d %#010lx" NEWLINE, fiu, cs, xfer->address);

	/* program commands are slow, warn the user */
	if (((xfer->cmd == FLASH_CMD_PP) || 
		(xfer->cmd == FLASH_CMD_4BYTE_PP)) && ((xfer->address & 0xFFFF) == 0))
		platform_printf("TIP PROG fiu%d cs%d %#010lx" NEWLINE, fiu, cs, xfer->address);
#endif
 ...
 */
uint32_t tip_flash_capabilities (const struct flash_master *spi)
{
	...
	struct spi_flash **recovery_flash, struct spi_flash **active_flash, uint32_t *recovery_offset)
{
	...
	*active_flash = NULL;
	...
	status = tip_flash_get_index_of_currently_running_image (&active_flash_ind, &fiu, &cs);
	if (status != 0) {
...
int tip_flash_mux_init (uint32_t fiu, uint32_t cs)
{
	...
	if (fiu == FIU_MODULE_1) {
		...
		CHIP_MuxFIU (FIU_MODULE_1, TRUE, TRUE, FALSE, FALSE, FALSE);
	}
	...
	if ((fiu == FIU_MODULE_2) || (fiu == FIU_MODULE_3)) {
		...
		CHIP_MuxFIU (FIU_MODULE_2, TRUE, TRUE, TRUE, TRUE, FALSE);
	}
