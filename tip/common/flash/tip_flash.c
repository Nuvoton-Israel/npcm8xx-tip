...
/**
 * Physical address of recovery image.
 */
static uint32_t recovery_phys_addr;

/**
 * Get recovery_phys_addr. Value can be obtained only after calling tip_flash_get_layout
 *
 * @return recovery_phys_addr
 */
...
/**
 * Get flash FIU and CS from spi_flash object.
 *
 * @param spi_fl Pointer to spi_flash.
 * @param fiu FIU index.
...
{
	struct tip_flash_master *fl_tip_m;

	if ((spi_fl == NULL) || (fiu == NULL) || (cs == NULL)) {
		return FLASH_MASTER_INVALID_ARGUMENT;
	}

	fl_tip_m = (struct tip_flash_master *) spi_fl->spi;
	*fiu = fl_tip_m->fiu;
	*cs = fl_tip_m->cs;
	return 0;
}
...
int tip_flash_xfer (const struct flash_master *spi, const struct flash_xfer *xfer)
{
...
#ifndef BMC_DIRECT
		/* set the flag indicating TIP xfer is in progress. */
		status = flash_xfer_tip_set_xfer_in_progress ();
		if (status != 0) {
			return status;
		}
#else
	/** TODO: Ensure there's a synchronization between flash xfers scheduled
	 * on the same FIU */
#endif
	...
	hal_status = FIU_UMA_ioctl (fiu, cs, xfer->cmd, xfer->address, addrBytes, xfer->data,
		(xfer->flags & FLASH_FLAG_DATA_TX) ? 0 : xfer->length, xfer->data,
		(xfer->flags & FLASH_FLAG_DATA_TX) ? xfer->length : 0, cmdBits, addrBits, dataBits,
		dataBits, FIU_TIMEOUT, dummy_bytes);
	...
	if (hal_status != DEFS_STATUS_OK) {
		platform_printf_dbg ("TIP_FLASH XFER failed fiu%d cs%d status %#010lx" NEWLINE, fiu, cs,
			hal_status);
...
#endif
#ifndef BMC_DIRECT
	flash_xfer_tip_clear_xfer_in_progress ();
#endif
...
static void tip_flash_init_data_array (void)
{
	...
	for (fiu = FIU_MODULE_0; fiu < FIU_MODULE_3; ++fiu) {
		for (cs = FIU_CS_0; cs <= FIU_CS_3; ++cs) {
			...
			tip_flash_master[bootIdx].size = 0;
		}
...
/**
 * Release spi_flash that was already init or failed the init flow.
 *
 * @param spi Index in the tip flash array
 * @return void
 */
static void tip_flash_release (uint32_t spi)
{
#ifdef _PLATFORM_RTOS_
	if (tip_flash_master[spi].size == 0) {
		return;
	}

	if (&tip_spi_flash[spi].state->lock != NULL) {
		spi_flash_release (&tip_spi_flash[spi]);
		tip_spi_flash[spi].state->lock = NULL;
		tip_flash_master[spi].size = 0;
	}
#endif
	return;
}
...
int tip_flash_initialize_access_single_flash (uint32_t fiu, uint32_t cs)
{
	...
	/* if this flash was already init, release it */
	tip_flash_release (bootIdx);
	...
	if (tip_flash_direct_read_quad (fiu)) {
		platform_printf ("fiu%d: 1-4-4 mode enabled" NEWLINE, fiu);
	}

	platform_printf_dbg (KGRN "%s complete fiu%d cs%d pass %x, flash size %#010lx" NEWLINE KNRM,
...
int tip_flash_initialize_access (uint32_t max_fiu, uint32_t max_cs)
{
...
#ifndef BMC_DIRECT
	/* Initialize the mutex for arbitration register access. */
	flash_xfer_tip_mutex_init ();
#endif
	...
	for (fiu = FIU_MODULE_0; fiu <= max_fiu; ++fiu) {
		...
		for (cs = FIU_CS_0; cs <= max_cs; ++cs) {
			...
			/* fiu 1 cs 1 is not supported as boot address in TIPROM */
			if ((fiu == 1) && (cs == 1)) {
				platform_printf ("Skip FIU1 CS1" NEWLINE);
				continue;
			}
			...
			platform_printf (KGRN "%s complete fiu%d cs%d pass %x, flash size %#010lx ID %x" NEWLINE KNRM,
				__func__, fiu, cs, status, tip_flash_master[bootIdx].size,
				*(uint32_t *) tip_spi_flash[bootIdx].state->device_id);
		}
	...
	*active_flash = tip_flash_get_spi_flash (active_flash_ind);
			...
			if (tip_flash_master[bootIdx].size >= _4MB_) {
				...
				if (flash_cnt == 1) {
					*recovery_flash = &tip_spi_flash[bootIdx];
					recovery_phys_addr = TIP_FLASH_ADDR_ARRAY [bootIdx];
	...
	if (flash_cnt == 1) {
		...
		*recovery_offset = (*main_flash)->state->device_size >> 1;
		recovery_phys_addr = TIP_FLASH_ADDR_ARRAY [0] + *recovery_offset;
 ...
 *
 * @return true if set to QUAD, false otherwise
 */
bool tip_flash_direct_read_quad (uint32_t fiu)
{
	...
	if (quad == true) {
		if (flash_FIU_addr_mode_state[fiu] == 4)
			read_cmd_default[fiu] = FLASH_CMD_4BYTE_QIO_READ;
		else
			read_cmd_default[fiu] = FLASH_CMD_QIO_READ;
		FIU_ConfigCommand (fiu, read_cmd_default[fiu] , 0);
		FIU_ConfigReadMode (fiu, FIU_FAST_READ_QUAD_IO);
		FIU_ConfigDummyCyclesCount (fiu, FIU_DUMMY_COUNT_3);
	}
							  ...
							  uint32_t *addr_stop, uint32_t num_or_allowed_regions)
{
	...
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_DIO_READ  , 3, 2, 1);
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_QUAD_READ , 3, 1, 1);
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_QIO_READ  , 3, 4, 1);
	...
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_4BYTE_DIO_READ , 4, 2, 1);
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_4BYTE_QUAD_READ, 4, 1, 1);
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_4BYTE_QIO_READ , 4, 4, 1);
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, FLASH_CMD_ALT_WRSR2 , 4, 1, 1); /* Quad input to program (0x3e) command for MXIC SPI */
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, 0x32 , 4, 1, 1); /* Quad input to program (0x3e) command for Winbond SPI */
	stat += FIU_SetProtectionCommand (fiu, cmd++, 0x7, 0x34 , 4, 1, 1); /* Quad input to program (0x3e) command for Winbond SPI */
