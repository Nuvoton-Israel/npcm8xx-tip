...
void hex_dump (uint32_t addr, uint32_t size, char *title)
{
	...
	for (uint32_t i = 0; i < size; i += 4) {
		...
		platform_printf ("%#010lx ", *(uint32_t *) (addr + i));
	}
 ...
 * @param  boot_addr: physical address in flash mem map to start searching a valid
 *                    image on the next boot.
 */
void tip_select_next_boot_image (uint32_t boot_addr)
{
	REG_WRITE (TIP_SEARCH_HDR_START,  boot_addr >> 16);
	platform_printf (KMAG "Next boot addr %#010lx (TIP_SCR0 %#010lx)" NEWLINE KNRM, boot_addr,
...
void tip_print_status (const char *ver)
{
	...
	if (ChipVersion == ARBEL_VERSION_A1)
		...
		platform_printf (KRED NEWLINE "=========" NEWLINE "Arbel Z1 NOT SUPPORTED" NEWLINE "=========" NEWLINE);
		while(1);
	}
	else if (ChipVersion == ARBEL_VERSION_A2)
		platform_printf (KGRN NEWLINE "=========" NEWLINE "Arbel A2" NEWLINE "=========" NEWLINE);
	else if (ChipVersion == ARBEL_VERSION_A3)
		platform_printf (KGRN NEWLINE "=========" NEWLINE "Arbel A3" NEWLINE "=========" NEWLINE);
	else
	...
	switch (data[0]) {
		...
		case 26:
			...
			break;
		case 27:
			platform_printf ("CoreWeave" NEWLINE);
			break;
		default:
...
void tip_stack_profiler (void)
{
	...
	extern uint32_t _ram_code_src_start;
	extern uint32_t _ram_code_dst_start ;
	extern uint32_t _ram_code_size;
	unsigned long *p;

	platform_printf (NEWLINE KGRN "Image stats" NEWLINE KNRM);
	platform_printf("code source= %#010lx" NEWLINE, &_ram_code_src_start);
	platform_printf("code copy  = %#010lx" NEWLINE, &_ram_code_dst_start);
	platform_printf("code size  = %#010lx" NEWLINE, &_ram_code_size);
	platform_printf("code end   = %#010lx" NEWLINE, 
		(uint32_t) &_ram_code_dst_start + (uint32_t) &_ram_code_size);
	
	p = &_stack_end;
	while (p < &_stack_start) {
		if (*p == (unsigned int)0xAAAAAAAA) {
			p++;
			}
		else {
			break;
	...
	platform_printf ("stack top       %#010lx" NEWLINE, &_stack_end);
	platform_printf ("stack watermark %#010lx" NEWLINE, p);
	platform_printf ("stack bottom    %#010lx" NEWLINE, &_stack_start);
	platform_printf ("stack usage     %#010lx" NEWLINE, stack_used);
	platform_printf ("stack usage percentage: %lu%%" NEWLINE, (stack_used * 100) / stack_total);
}
#endif
