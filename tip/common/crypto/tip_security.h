...
int  tip_image_hash (const struct hash_engine *hash_engine, HEADER_GENERAL_T* imgHeaderInRAM, uint8_t* imageAddr, uint32_t imageLen, uint8_t* hash, uint8_t hashAlg);
int  tip_image_auth_ECC (const struct ecc_hw *ecc_engine, HEADER_GENERAL_T* imgHeaderInRAM, uint8_t* imageAddr, uint32_t imageLen, uint8_t* key, uint8_t* hash, SECURED_BOOLEAN_T *sigOK, uint8_t hashAlg);
