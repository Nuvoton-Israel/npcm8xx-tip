 ...
 */
uint32_t tip_mbx_get_notification (void)
{
	uint32_t notification = MAKE32 (REG_READ (B2TIPST0), REG_READ (B2TIPST1));
	return notification;
}
...
void tip_mbx_clear_notification (uint32_t notification)
{
	...
	/* whenever BMC is reset need to clear all notification and the first packet
	 * counter
	 */
	if (notification == 0xFFFFFFFF) {
		packet_counter = 0;
	}
}
 ...
 */
int tip_receive_packet (const struct cmd_channel *channel, struct cmd_packet *packet,
	int ms_timeout)
{
	const struct tip_cmd_channel *tip_cmd_cnl = (const struct tip_cmd_channel*) channel;
	...
	/* clear notification bit that remains set even in polling mode */
	NVIC_ClearInt (NVIC_INT_2);

	if (packet_counter < (BMC_COMBO1_MAX_COMPONENT_COUNT + 1)) {
		packet_counter++;
 ...
 */
int tip_send_packet (const struct cmd_channel *channel, const struct cmd_packet *packet)
{
	const struct tip_cmd_channel *tip_cmd_cnl = (const struct tip_cmd_channel*) channel;
...
int tip_cmd_channel_init (struct tip_cmd_channel *channel, int id, uint32_t addr, uint32_t size)
{
	...
	status = cmd_channel_init (&channel->base, &channel->channel_state, id);
	if (status != 0) {
		return status;
	}
