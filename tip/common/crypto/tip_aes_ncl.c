	...
	size_t out_length, uint8_t *tag, size_t tag_length)
{
	...
	tip_ncl->shared_state->lock (tip_ncl->shared_state);
	ncl_status = pROM_NCL_AES_SetParams (tip_ncl->context, NULL, key_length, tip_ncl->aes_mode,
		(uint32_t*) iv);
...
static int tip_aes_select_key (struct tip_aes_ncl_engine *engine, uint8_t key_num)
{
	...
	NCL_STATUS_RET_CHECK_MUTEX (pROM_NCL_AES_SelectKey (engine->context, engine->key_ind),
		engine->shared_state);
...
int tip_aes_ncl_init (struct tip_aes_ncl_engine *engine, struct tip_rom_ncl_shared_state *shared_state)
{
	...
	engine->shared_state->lock (engine->shared_state);
	NCL_STATUS_RET_CHECK_MUTEX (pROM_NCL_AES_Power (engine->context, TRUE),
		engine->shared_state);

	NCL_STATUS_RET_CHECK_MUTEX (pROM_NCL_AES_InitContext (engine->context),
		engine->shared_state);

	NCL_STATUS_RET_CHECK_MUTEX (pROM_NCL_AES_Init (engine->context),
		engine->shared_state);
	engine->shared_state->unlock (engine->shared_state);
