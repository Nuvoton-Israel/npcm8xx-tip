...
;
    ...
    ots_type = get_bigendian( public_key+4, 4 );
...
failed:
    /* If we get an intermediate failure */
...
DEFS_STATUS hss_validate_signature_finalize(
    struct hss_validate_inc *ctx,
    const unsigned char *signature,
    struct hss_extra_info *info) {

    volatile DEFS_STATUS      isSigValid = DEFS_STATUS_FAIL;
    ...
    tempRes = lm_validate_signature(ctx->final_public_key, hash, sizeof hash, true,
                                    signature + ctx->signature_offset, ctx->signature_len);

    DEFS_FLOW_MONITOR_INCREMENT();
    if (DEFS_SEC_COND_CHECK(tempRes == DEFS_STATUS_OK))
    {
        isSigValid = DEFS_STATUS_OK;
        DEFS_FLOW_MONITOR_INCREMENT();
    }
    else
    {
    ...
    DEFS_FLOW_MONITOR_COMPARE_RET(3);
    return isSigValid;
}
