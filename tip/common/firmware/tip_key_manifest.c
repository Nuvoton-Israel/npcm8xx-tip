	...
	const struct hash_engine *hash)
{
	...
	status = tip_firmware_component_verify (kmt->fw, hash, NULL, NULL, NULL, 0, kmt->ecc);
...
static int tip_key_manifest_check_key (uint8_t *data_kmt, uint16_t key_size, int num_of_keys, const char *key_type, uint16_t start_from)
{
	for (int kmt_key_cnt = 0; kmt_key_cnt < num_of_keys; ++kmt_key_cnt) {
		uint16_t valid_kmt = *(uint16_t *)(data_kmt + (key_size - 2));
		platform_printf ("KMT %s public key %d is 0x%x-0x%x-0x%x-..., key %s valid" NEWLINE,
									 key_type, kmt_key_cnt, (data_kmt)[start_from], (data_kmt)[start_from + 1], (data_kmt)[start_from + 2] , 
									 valid_kmt != 0x3A3A ? "not" : "");
        if (valid_kmt != 0x3A3A && strcmp(key_type ,"ECC" ) == 0) {
				/* only on ECC it is considered a failure if the KMT key's valid is not valid*/
	...
	const uint8_t *verified_kmt, uint32_t kmt_copy_addr, bool copy_to_ram)
{
...
#ifdef LMS_ENABLE
	{
		...
		if (LMS_Start_offset > 0) {
			...
			/* check and print KMT LMT keys and their valid */
			tip_key_manifest_check_key ((uint8_t *) KMT_HeaderRamPtr + sizeof (IMG_HEADER_T) + LMS_Start_offset,
			LMS_KEY_SIZE, num_of_lms_keys, "LMS", 8);
	...
	if (num_of_ecc_keys > 0) {
		/* check and print KMT ECC keys and their valid */
		status = tip_key_manifest_check_key ((uint8_t *) KMT_HeaderRamPtr + sizeof (IMG_HEADER_T),
		ECC_KEY_SIZE, num_of_ecc_keys, "ECC", 0);
		if (status) {
			return KEY_MANIFEST_WEAK_KEY;
		}
	}
...
int tip_invalidate_otp_keys (void)
{
	...
	uint32_t key_invalid = REG_READ (TIP_INVALIDATE_OTP_KEYS_SCR);

	/* if no key needs to be invalidated - return */
	if (key_invalid == 0) {
		return 0;
	}
