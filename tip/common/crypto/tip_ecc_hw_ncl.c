	...
	const uint8_t *priv_key, size_t key_length, struct ecc_point_public_key *pub_key)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_InitContext (tip_ncl->state->context));

	tip_ecc_get_curve_data(&curve, tip_ncl->state->context, curve_type, key_length);

	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_SetCurve (tip_ncl->state->context, curve));
	...
	size_t key_length, uint8_t *priv_key, struct ecc_point_public_key *pub_key)
{
	...
	tip_ecc_get_curve_data (&curve, tip_ncl->state->context, curve_type, key_length);
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_SetCurve (tip_ncl->state->context, curve));
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_SetSeed (tip_ncl->state->context, 0xffffffff));
	ncl_status = pROM_NCL_ECC_GenKey (tip_ncl->state->context, &prv_key_ncl, &pub_key_ncl, curve,
		NULL, NULL);
	...
	const uint8_t *digest, size_t digest_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_SetCurve (tip_ncl->state->context, curve));
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_SetSeed (tip_ncl->state->context, 0xffffffff));
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_ECDSAVerifyHash (tip_ncl->state->context, &sigOk, &rSig,
		&sSig, hash_alg, digest, &pub_key_ncl, curve));
...
{
	const struct tip_ecc_hw_ncl_engine *engine = (const struct tip_ecc_hw_ncl_engine*) ecc_hw;
...
int tip_ecc_hw_ncl_init_state (const struct tip_ecc_hw_ncl_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_Power (engine->state->context, TRUE));
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_InitContext (engine->state->context));
	NCL_STATUS_RET_CHECK (pROM_NCL_ECC_Init (engine->state->context));
