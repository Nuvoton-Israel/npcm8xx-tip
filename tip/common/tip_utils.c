...
void tip_print_status (const char *ver)
{
	...
	if (data[0] == 1)
		...
		platform_printf ("Unknown" NEWLINE);

	FUSE_WRPR_get (SERIAL_NUMBER_PROPERTY, data);

	platform_printf ("Chip serial number is  0x%x-0x%x-0x%x-0x%x" NEWLINE, data[3], data[2],
		data[1], data[0]);
