...
static int initialize_dice ()
{
	...
	if ((*(uint32_t *)DICE_CDI == 0) && (*(uint32_t *)(DICE_CDI + 4) == 0)) {
		int i;
		platform_printf (KRED "NO DICE IDENTITY ON DEVICE! CHIP CAN BE USED FOR DEBUG ONLY" NEWLINE KNRM);
		for (i = 0; i < SHA512_HASH_LENGTH; i++) {
			*(DICE_CDI + i) = i;
		}
	}
