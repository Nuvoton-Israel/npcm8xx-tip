 ...
 */
DEFS_STATUS lm_validate_signature(
    const unsigned char *public_key,
    const void *message, size_t message_len, bool prehashed,
    const unsigned char *signature, size_t signature_len) {

    union    hash_context       ctx;
    volatile DEFS_STATUS        retStatus = DEFS_STATUS_FAIL;
    volatile SECURED_BOOLEAN_T  tempRes   = SECURED_FALSE;
    ...
    unsigned h, n, height;
    if (!lm_look_up_parameter_set(lm_type, &h, &n, &height)) return retStatus;
	serial_printf("line is %d  \n", __LINE__);

    unsigned char computed_public_key[MAX_HASH];

    const unsigned char *I = public_key + LM_PUB_I;

    if (signature_len < 8) return retStatus;
	serial_printf("signature_len line %d is %d  \n", __LINE__,  signature_len);
    ...
    if (ots_publen == 0 || ots_siglen == 0) return retStatus;
		serial_printf("line is %d  \n", __LINE__);
    if (signature_len < ots_siglen) return retStatus;
	serial_printf("signature_len line %d is %d  \n", __LINE__,  signature_len);
    ...
    if (!lm_ots_validate_signature_compute(ots_sig + LEAF_PK, I, count,
              ...
              signature, ots_siglen, ots_type)) return retStatus;
	serial_printf("line is %d  \n", __LINE__);
    signature += ots_siglen; signature_len -= ots_siglen;

    /* Get the parameter set declared in the sigature; make sure it matches */
    /* what we expect */
    if (signature_len < 4) return retStatus;
	serial_printf("signature_len line %d is %d  \n", __LINE__,  signature_len);
    param_set_t parameter_set = get_bigendian( signature, 4 );
	serial_printf("parameter_set is %d  \n", parameter_set);
	serial_printf("lm_type is %d  \n", lm_type);
    if (parameter_set != lm_type) return retStatus;
	serial_printf("line is %d  \n", __LINE__);

    signature += 4; signature_len -= 4;

    merkle_index_t count_nodes = (merkle_index_t)1 << height;
	serial_printf("n  is %d  \n", n ); //32

	serial_printf("signature_len line %d is %d  \n", __LINE__,  signature_len);
	serial_printf("height  is %d  \n", height); //H = 20
	serial_printf("signature_len-n*height   is %d  \n", signature_len-(n*height));
			

    if (signature_len != n * height) return retStatus;  /* We expect the auth */
		serial_printf("line is %d  \n", __LINE__);
                                    /* path to be there as the last element */
    if (count >= count_nodes) return retStatus;  /* Index out of range */
		serial_printf("line is %d  \n", __LINE__);
    ...
    tempRes = SECURE_MEMCMP(computed_public_key, public_key + offset, n);
    DEFS_FLOW_MONITOR_INCREMENT();

    if (DEFS_SEC_COND_CHECK(tempRes == SECURED_TRUE))
    {
