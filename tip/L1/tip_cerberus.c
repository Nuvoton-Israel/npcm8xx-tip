 ...
 */
static struct spi_flash virtual_flash;
...
static int initialize_riot_manager ()
{
	...
	status = flash_store_contiguous_blocks_init_variable_storage_decreasing (&keystore_flash,
		&main_flash->base, main_keystore_addr, MAIN_KEYSTORE_MAX_KEYS, 0, &shared_hash.base);
	if (status != 0) {
		return status;
	}
...
static void cerberus_init (void *unused)
{
	...
	status = tip_firmware_component_init (&skmt_img, active_flash, IMG_SKMT, -1);
	if (status != 0) {
	...
	status = tip_load_bmc_firmware (active_flash,
	...
	SET_REG_FIELD (TIP_SEARCH_HDR_START, TIP_SEARCH_HDR_ADDR, 0);
