 ...
 * Relocating KMT is to save space for heap.
 *
...
int tip_key_manifest_parse_and_copy (struct tip_primary_key_manifest *kmt,
	const uint8_t *verified_kmt, uint32_t kmt_copy_addr)
{
	IMG_HEADER_T *KMT_HeaderRamPtr;
	uint8_t num_of_kmt_keys;
	uint8_t *data_kmt;
	uint16_t valid_kmt;

	if (kmt == NULL || verified_kmt == NULL || kmt_copy_addr < TIP_RAM_BASE_ADDR ||
	...
	/* 128 bytes per key */
	num_of_kmt_keys = KMT_HeaderRamPtr->header.img_length >> 7;
	if (num_of_kmt_keys > 2) {
		platform_printf (KRED "ERROR: only two KMT keys are allowed" NEWLINE KNRM);
		return KEY_MANIFEST_INVALID_FORMAT;
	}

	for (int kmt_key_cnt = 0; kmt_key_cnt < num_of_kmt_keys; ++kmt_key_cnt) {
