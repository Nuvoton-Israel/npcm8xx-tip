...
static NCL_STATUS_T set_params (struct tip_aes_ncl_engine *tip_ncl, const uint8_t *iv, size_t iv_or_key_length,
...
static int tip_aes_ncl_encrypt_data (struct aes_engine *engine, const uint8_t *plaintext,
	size_t length, const uint8_t *iv, size_t iv_or_key_length, uint8_t *ciphertext, size_t out_length,
...

{
	...
	if (tip_ncl->aes_mode == NCL_AES_MODE_GCM) {
		...
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, tag_length,
		...
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, 0,
			(NCL_AES_OP_T) NCL_AES_OP_ENCRYPT);
		if (ncl_status != NCL_STATUS_OK) {
			return AES_ENGINE_NO_KEY;
		}
...
static int tip_aes_ncl_decrypt_data (struct aes_engine *engine, const uint8_t *ciphertext,
	size_t length, const uint8_t *tag, const uint8_t *iv, size_t iv_or_key_length, uint8_t *plaintext,
	size_t out_length)
{
	struct tip_aes_ncl_engine *tip_ncl = (struct tip_aes_ncl_engine *) engine;
	NCL_STATUS_T ncl_status;
	...
	if (tip_ncl->aes_mode == NCL_AES_MODE_GCM) {
		...
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, AES_GCM_TAG_LEN,
