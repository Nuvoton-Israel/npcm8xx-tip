...
int tip_update_device_prop (void)
{
	...
	status = FUSE_WRPR_get(LIFE_CYCLE_ENC_PROPERTY, &oLifeCycle);
	if (status != DEFS_STATUS_OK)
		oLifeCycle = LIFE_CYCLE_UNDEFINED;
	else
	{
		// The Life Cycle state should be determined by OR function between the bit in the LSB and the bit in the MSB
		oLifeCycle = (oLifeCycle | (oLifeCycle >> 4)) & 0x0F;
	}

	// avoid cases in which a clear cycle mode (new chip) may accidently fuse the OTP values
	if (oLifeCycle != LIFE_CYCLE_UNTESTED && oLifeCycle != LIFE_CYCLE_NO_TIP) {
		while (count > 0) {
			/* program the OTP */
		 ...
		 */
		if (((status1 == DEFS_STATUS_OK) && (status2 == DEFS_STATUS_OK))) {
			platform_printf (KGRN "OTP patch applied successfully" KNRM NEWLINE);
			tip_select_next_boot_image (SPI0CS0_BASE_ADDR); /* set next boot to active flash */
			platform_reset (0);	/* reset to make OTP take effect */
		}
		else {
