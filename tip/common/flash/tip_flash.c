...
int tip_flash_xfer (const struct flash_master *spi, const struct flash_xfer *xfer)
{
	...
	if ((xfer->cmd == FLASH_CMD_READ) ||
		...
		if (READ_REG_FIELD (FIU_DRD_CFG(fiu), FIU_DRD_CFG_RDCMD) != xfer->cmd) {
			...
			platform_printf_dbg (NEWLINE "tip restore read command FIU_DRD_CFG[%d] %#010lx" NEWLINE KNRM, 
	...
	if((xfer->cmd == FLASH_CMD_4BYTE_4K_ERASE) || 
		...
		if (xfer->address == 0x07420000)  {

			SET_REG_FIELD(FLOCKR1, FLOCKR1_JTAGDIS, 0) ;
			SET_REG_FIELD(TIP_DBG_CTL, TIP_DBG_CTL_TIP_JEN, TIP_SEC_TRUE);
			SET_REG_FIELD(MFSEL7, MFSEL7_TPJTAG3SEL, 1);
		}
