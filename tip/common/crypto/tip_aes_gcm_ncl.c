	...
	size_t auth_data_length, NCL_AES_OP_T op)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_AES_SelectKey (tip_ncl->state->context, 0));

	return pROM_NCL_AES_GCM_SetParams (tip_ncl->state->context, (uint32_t *) iv, NULL,
...
int tip_aes_gcm_ncl_encrypt_with_add_data (const struct aes_gcm_engine *engine,
	...
	size_t out_length, uint8_t *tag, size_t tag_length)
{
	return tip_aes_gcm_ncl_encrypt_data_common (engine, plaintext, length, iv, iv_or_key_length,
...
int tip_aes_gcm_ncl_encrypt_data (const struct aes_gcm_engine *engine, const uint8_t *plaintext,
	...
	size_t out_length, uint8_t *tag, size_t tag_length)
{
	return tip_aes_gcm_ncl_encrypt_data_common (engine, plaintext, length, iv, iv_or_key_length, NULL,
...
int tip_aes_gcm_ncl_init_state (const struct tip_aes_gcm_ncl_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_AES_Power (engine->state->context, TRUE));
	NCL_STATUS_RET_CHECK (pROM_NCL_AES_InitContext (engine->state->context));
	NCL_STATUS_RET_CHECK (pROM_NCL_AES_Init (engine->state->context));
...
int tip_aes_gcm_ncl_init (struct tip_aes_gcm_ncl_engine *engine,
	struct tip_aes_gcm_ncl_state *state)
{
	if (engine == NULL)
		return AES_GCM_ENGINE_INVALID_ARGUMENT;
