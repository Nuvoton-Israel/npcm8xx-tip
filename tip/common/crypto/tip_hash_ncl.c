	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->state->context.ncl_sha));
	status = tip_security_sha_calc (tip_ncl->state->context.ncl_sha, NCL_SHA_TYPE_2_256, data, length,
		hash);
	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->state->context.ncl_sha));
	status = tip_security_sha_calc (tip_ncl->state->context.ncl_sha, NCL_SHA_TYPE_2_384, data, length,
		hash);
	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->state->context.ncl_sha));
	status = tip_security_sha_calc (tip_ncl->state->context.ncl_sha, NCL_SHA_TYPE_2_512, data, length,
		hash);
...
int tip_hash_ncl_init_state (const struct tip_hash_ncl_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Power (engine->state->context.ncl_sha, TRUE));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_InitContext (engine->state->context.ncl_sha));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Init (engine->state->context.ncl_sha));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (engine->state->context.ncl_sha));
