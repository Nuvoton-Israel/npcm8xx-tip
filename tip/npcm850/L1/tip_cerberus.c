...
#ifdef CERBERUS_ENABLE_COMPONENT_ATTESTATION
	status = system_state_manager_init (&system_state, &main_flash->base, SYSTEM_STATE_ADDR);
	if (status != 0) {
		return status;
	}
#endif
...
static int initialize_pcd_management ()
{
	...
	status = pcd_flash_init (&pcd_region1, &main_flash->base, &shared_hash.base,
		pcd_region1_addr, pcd_region1_signature, sizeof (pcd_region1_signature),
	...
	status = pcd_flash_init (&pcd_region2, &main_flash->base, &shared_hash.base,
		pcd_region2_addr, pcd_region2_signature, sizeof (pcd_region2_signature),
	...
	platform_printf ("PCD: active=0x%p" NEWLINE, active_pcd);
...
static int initialize_component_management ()
{
	...
	status = cfm_flash_init (&cfm_region1, &main_flash->base, &shared_hash.base,
		cfm_region_addr[0], cfm_region1_signature, sizeof (cfm_region1_signature),
	...
	status = cfm_flash_init (&cfm_region2, &main_flash->base, &shared_hash.base,
		cfm_region_addr[1], cfm_region2_signature, sizeof (cfm_region2_signature),
	...
	platform_printf ("CFM: active=0x%p, pending=0x%p" NEWLINE, active, pending);
