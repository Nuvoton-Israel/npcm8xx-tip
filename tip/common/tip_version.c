...
uint16_t tip_get_otp_version (IMG_TYPE_E img)
{
	...
	uint16_t version;
	uint16_t bits_per_version = 2;
	uint8_t bits_mask = 0xC0;
...
int tip_version_set_in_OTP (struct tip_version_handler *ver)
{
	...
	for (IMG_TYPE_E img = IMG_KMT; img <= IMG_UBOOT; img = img << 1) {
		...
		if (next_minimal_version <= current_fw_version &&
			...
			uint8_t fcfg_val = READ_REG_FIELD (FCFG0_7 (block/8), FCFG0_7_FPRGDIS);
			SET_REG_FIELD (FCFG0_7 (block/8), FCFG0_7_FPRGDIS, fcfg_val & ~(1<<(block % 8)));
			/*
			...
			FUSE_ProgramBit (0, address + (bitOffset / 8), (bitOffset % 8), FALSE);
			FUSE_ProgramBit (0, address + (bitOffset / 8), (bitOffset % 8) + 1, FALSE);
			/* disable FCFG once done */
			SET_REG_FIELD (FCFG0_7 (block/8), FCFG0_7_FPRGDIS, fcfg_val);
