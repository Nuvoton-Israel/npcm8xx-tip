...
/**
 * TODO : Refactor common pieces between AES_GCM and AES_GCM_NCL and remove duplicate functions.
 * ADO : https://dev.azure.com/NuvotonCerberus/Arbel/_workitems/edit/205
 */
...
static NCL_STATUS_T set_params (struct tip_aes_ncl_engine *tip_ncl, const uint8_t *iv,
	size_t iv_or_key_length, size_t tag_length, const uint8_t *auth_data, size_t auth_data_length,
	NCL_AES_OP_T op)
{
	if (tip_ncl->aes_mode == NCL_AES_MODE_GCM) {
		...
		AESaddedinfoGCM.ivSize = (UINT32) iv_or_key_length;
		AESaddedinfoGCM.macSize = tag_length_enum;

		NCL_STATUS_RET_CHECK (pROM_NCL_AES_SelectKey (tip_ncl->context, tip_ncl->key_ind));

		return pROM_NCL_AES_GCM_SetParams (tip_ncl->context, (uint32_t *) iv, NULL,
			tip_ncl->key_size, (const NCL_AES_ADD_DATA*) &AESaddedinfoGCM, op);
	}
	else {
		return pROM_NCL_AES_SetParams (tip_ncl->context, NULL, iv_or_key_length, tip_ncl->aes_mode,
			(uint32_t*) iv);
	}
}
	...
	size_t out_length, uint8_t *tag, size_t tag_length)
{
	...
	if (tip_ncl->aes_mode == NCL_AES_MODE_GCM) {
		...
		/* fail in case there is no key */
		if (tip_ncl->key_size == 0) {
			return TIP_AES_ENGINE_NO_KEY;
		}
		if (tag_length < 16 || iv_or_key_length == 0) {
			return TIP_AES_ENGINE_OUT_BUFFER_TOO_SMALL;
		}
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, tag_length, additional_data,
			additional_data_length, (NCL_AES_OP_T) NCL_AES_OP_ENCRYPT);
		...
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, 0, additional_data,
			additional_data_length, (NCL_AES_OP_T) NCL_AES_OP_ENCRYPT);
		if (ncl_status != NCL_STATUS_OK) {
...
static int tip_aes_ncl_encrypt_with_add_data (const struct aes_gcm_engine *engine,
	...
	size_t out_length, uint8_t *tag, size_t tag_length)
{
	return tip_aes_ncl_encrypt_data_common (engine, plaintext, length, iv, iv_or_key_length,
		additional_data, additional_data_length , ciphertext, out_length, tag, tag_length);
}

/*
* @param iv_or_key_length is iv_length when it's GCM mode, and key_length for all other modes (CBC\ CTR \ etc)
*/
...
{
	return tip_aes_ncl_encrypt_data_common (engine, plaintext, length, iv, iv_or_key_length, NULL,
...
static int tip_aes_ncl_decrypt_data_common (const struct aes_gcm_engine *engine,
	...
	size_t iv_or_key_length, const uint8_t *addtional_data, size_t addtional_data_length,
	uint8_t *plaintext, size_t out_length)
{
	...
	if (tip_ncl->aes_mode == NCL_AES_MODE_GCM) {
		if (tag == NULL || iv_or_key_length == 0) {
			return TIP_AES_ENGINE_INVALID_ARGUMENT;
		}
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, AES_GCM_TAG_LEN, addtional_data,
		...
		if (ncl_status == NCL_STATUS_AUTHENTICATION_FAIL) {
			return tip_ncl_status (ncl_status, TIP_AES_ENGINE_GCM_AUTH_FAILED);
		}
	} else {
		/* For Non-GCM operations, additional_data is NULL. */
		ncl_status = set_params (tip_ncl, iv, iv_or_key_length, 0, NULL, 0,
...
{
	return tip_aes_ncl_decrypt_data_common (engine, ciphertext, length, tag, iv, iv_length, NULL,
