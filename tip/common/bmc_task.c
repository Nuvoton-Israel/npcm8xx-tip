...
#ifdef BMC_DIRECT
static void tip_bmc_direct_finalize_command (int status)
{
...
void NVIC_BMC_direct_handler (uint16_t num)
{
	...
	platform_printf_dbg (KGRN NEWLINE
		"======== TIP_FW: detected BMC int %d notification %#010lx cmd 0x%x" NEWLINE KNRM,
		...
		/* handle post boot BMC reset event */
			...
			switch (notification) {
				...
				case BMC_DIRECT_COMMAND_FL_PROG: {
					...
					tip_bmc_direct_finalize_command (status);
					break;
				}
...
void bmc_export_data (void)
{
	...
	hex_dump((uint32_t)riot->keys_hash, sizeof (riot->keys_hash), "KMT_HASH");
	hex_dump((uint32_t)riot->riot_hash, sizeof (riot->riot_hash), "TFT_HASH");
	...
	/* Copy second portion of RIoT struct */
	size_to_copy = sizeof (struct riot_shared_attestation) - offsetof (struct riot_shared_attestation, alias_cert);
