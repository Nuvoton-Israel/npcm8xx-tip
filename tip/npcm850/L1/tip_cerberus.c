...
static void hardware_app_init ()
{
	...
	if (tip_L1_sys_ctrl.tip_print_to_memory) {
		uint32_t start_addr = REG_READ (SCRPAD_10_41(17));
		uint32_t size = REG_READ (SCRPAD_10_41(18));
		if (tip_check_mem_log_valid_location (start_addr, size) == true) {
			uint32_t next_addr  = REG_READ (SCRPAD_10_41(19));
			/* pull the information from the scratch pad and save it in the global */
			memory_printf_udpate (start_addr, size, next_addr);
		}
		else {
			tip_L1_sys_ctrl.tip_print_to_memory = false;
		}
	}
...
static int initialize_crypto ()
{
	...
	status = tip_rng_ncl_init (&system_rng, &system_rng_state);
	if (status != 0) {
		return status;
	}
...
static int initialize_session_management ()
{
	...
	status = tip_aes_gcm_ncl_init (&session_aes, &session_aes_context);
	if (status != 0) {
		return status;
	}
