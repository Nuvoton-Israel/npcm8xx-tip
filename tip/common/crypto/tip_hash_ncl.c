...
{
	pROM_NCL_SHA_FinalizeContext (engine->context);
}
	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Calc (tip_ncl->context, NCL_SHA_TYPE_2_256, data, length,
		hash);

	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
...
static int tip_hash_ncl_start_sha256 (struct hash_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Start (tip_ncl->context, NCL_SHA_TYPE_2_256);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Calc (tip_ncl->context, NCL_SHA_TYPE_2_384, data, length,
		hash);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
...
static int tip_hash_ncl_start_sha384 (struct hash_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Start (tip_ncl->context, NCL_SHA_TYPE_2_384);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
#endif
	...
	size_t length, uint8_t *hash, size_t hash_length)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Calc (tip_ncl->context, NCL_SHA_TYPE_2_512, data, length,
		hash);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
...
static int tip_hash_ncl_start_sha512 (struct hash_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (tip_ncl->context));
	NCL_STATUS_T status = pROM_NCL_SHA_Start (tip_ncl->context, NCL_SHA_TYPE_2_512);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
#endif
...
static int tip_hash_ncl_update (struct hash_engine *engine, const uint8_t *data, size_t length)
{
	...
	NCL_STATUS_T status = pROM_NCL_SHA_Update (tip_ncl->context, data, length);
	return tip_ncl_status (status, HASH_ENGINE_NO_ACTIVE_HASH);
}
...
static int tip_hash_ncl_finish (struct hash_engine *engine, uint8_t *hash, size_t hash_length)
{
	...
	status = pROM_NCL_SHA_Finish (tip_ncl->context, hash);

	if (status == NCL_STATUS_OK) {
		NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Clear (tip_ncl->context));
		NCL_STATUS_RET_CHECK (pROM_NCL_SHA_FinalizeContext (tip_ncl->context));
	}
...
static void tip_hash_ncl_cancel (struct hash_engine *engine)
{
	...
	if (tip_ncl) {
		pROM_NCL_SHA_Clear (tip_ncl->context);
	}
}
...
int tip_hash_ncl_init (struct tip_hash_ncl_engine *engine)
{
	...
	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Power (engine->context, TRUE));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_InitContext (engine->context));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Init (engine->context));

	NCL_STATUS_RET_CHECK (pROM_NCL_SHA_Reset (engine->context));
