	...
	for (int i = 0; i < TIP_COMBO_MAX_COMPONENT_COUNT; ++i) {
		...
		switch (component->img_type) {
			case IMG_KMT:
				...
				/* Propagate the ECC engine to the KMT for its verify callback. */
				fw_combo->kmt.ecc = fw_combo->ecc;
...
static int tip_image_combo_verify_func (const struct firmware_image *fw,
	const struct hash_engine *hash)
{
	...
	for (int i = 0; i < TIP_COMBO_MAX_COMPONENT_COUNT; ++i) {
		...
		status = tip_firmware_component_verify (&fw_combo->img[i], hash, &fw_combo->kmt,
			&fw_combo->skmt, NULL, 0, fw_combo->ecc);
		if (status != 0 && TIP_SECBOOT_IS_ACTIVE ()) {
			return status;
		}
		else {
			/* SECURE_BOOT disabled, ignore the verification failure. */
			status = 0;
		}
	}
...
static int tip_image_combo_get_image_size_func (const struct firmware_image *fw)
{
	...
	for (int i = 0; i < TIP_COMBO_MAX_COMPONENT_COUNT; ++i) {
		...
		/* In Combo 2 Bootblock starts from a fixed offset 512KB\2MB. The padding should be counted in
		 * the final image size. Adjust the calcuated Combo 0 size according to combo 1
		 *  offset from start of image */
		if (fw_combo->img[i].img_type == IMG_BOOTBLOCK && size > 0) {
			size = (uint32_t)fw_combo->img[i].header_flash & (ROT_IMAGE_MAX_SIZE - 1);
		}

#ifdef LMS_ENABLE
		/* after verification with ECC key, moving to verification with LMS key */
		if (fw_combo->img[i].header_flash->header.enableLMS == 0x01 || fustrap1_lms_enabeld 
			 || tip_is_kmt_and_LMS_is_forced_in_otp(&fw_combo->img[i]) ){
			fw_combo->img[i].is_lms_sig_exists = true;
			size += LMS_SIG_SIZE;
		} else {
			fw_combo->img[i].is_lms_sig_exists = false;
		}
#endif
