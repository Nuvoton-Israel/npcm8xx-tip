	...
	uint8_t *imageAddr, uint32_t imageLen)
{
	...
	for (key_index = key_index_min; key_index < key_index_max; key_index++) {
		...
		if (isECC) {
			status = tip_image_auth_ECC (header, imageAddr, imageLen, pub_key, digest_out, &sigOK);
		...
		/* perform LMS verification */
			...
			status = LMS_VerifySig (pub_key, imageAddr, imageLen, sig_arr, LMS_SIG_SIZE);
			if (status == DEFS_STATUS_OK) {
				sigOK = SECURED_TRUE;
			}
		}
	...
	uint8_t *digest_out, size_t digest_length)
{
...
#ifdef LMS_ENABLE
	{
		...
		if (header->header.enableLMS == 0x01 || READ_VAR_FIELD (fustrap1, FUSTRAP1_oLMS_AUTH_EN)) {
